...
const TeacherDashboard = React.lazy(() =>
  import('./components/teacher/TeacherDashboard').then((mod) => ({
    default: mod.TeacherDashboard,
  }))
);
const StudentView = React.lazy(() =>
  import('./components/student/StudentView').then((mod) => ({
    default: mod.StudentView,
  }))
);
const PollHistory = React.lazy(() =>
  import('./components/teacher/PollHistory').then((mod) => ({
    default: mod.PollHistory,
  }))
);
      ...
      if (isStudentRoute && sessionId) {
        ...
        socket.emit(
          ...
          (state: {
            ...
            poll?: Poll;
            remainingTime?: number;
            vote?: { hasVoted?: boolean; votedOptionId?: string };
          }) => {
            if (state && !state.error && state.poll) {
